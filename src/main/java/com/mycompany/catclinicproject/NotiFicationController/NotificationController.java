...
public class NotificationController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        NotificationDTO noti = dao.getNotificationByID(NotiID);
        String type = noti.getType();
        int medicalRecord = noti.getRefID();
        dao.markAsRead(NotiID);
        if (type.equalsIgnoreCase("EMR")) {
            response.sendRedirect("EmrController?medicalRecordID=" + medicalRecord);
        } else if (type.equalsIgnoreCase("request BL")) {
            response.sendRedirect("bloodtest?medicalRecordID=" + medicalRecord);
