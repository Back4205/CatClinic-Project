                ...
                <tbody>
                    <c:forEach items="${newList}" var="n">
                        <tr>
                            ...
                            <!-- Title -->
                            <td class="text-bold text-break">
                                <c:out value="${n.title}" />
                            </td>

                            <!-- Description -->
                            <td class="text-break">
                                <c:out value="${n.description.length() > 80 
                                    ? n.description.substring(0,80).concat('...') 
                                    : n.description}" />
                            </td>
                                ...
                                <div class="news-action-group">
                                    ...
                                    <c:choose>
                                        <c:when test="${n.isActive}">
                                            <a href="ChangeStatus?id=${n.newsId}" 
                                               class="btn-news-action toggle btn-toggle-hide">
                                                <i class="fa-solid fa-eye-slash"></i> Hide
                                            </a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="ChangeStatus?id=${n.newsId}" 
                                               class="btn-news-action toggle btn-toggle-show">
